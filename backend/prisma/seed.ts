...
async function main() {
  try {
    ...
    const galleryItems = [
      ...
      {
        title: 'Colorful Cookie Collection',
        category: 'Cookies',
        image: '/api/placeholder/400/300',
        featured: true,
        uploaderId: admin.id
    ...
    for (const item of galleryItems) {
      await prisma.galleryItem.upsert({
        where: { 
          // Since GalleryItem might not have a unique field, create a compound unique identifier
    ...
    const contactMessages = [
      ...
      },
      {
        ...
        message: 'I would like to order pastries for a corporate event. Can you provide catering for 50 people?',
